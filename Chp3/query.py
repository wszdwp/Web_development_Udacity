...
#ref  sorted(student_tuples, key=lambda student: student[2])
#print sorted([Link for Link in links if Link.submitter_id == 62443], key=lambda Link: Link.submitted_time)
def query():
    return sorted([Link for Link in links if Link.submitter_id == 62443], key=lambda Link: Link.submitted_time)
